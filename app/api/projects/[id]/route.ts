...
import { isProjectColor, isUniqueViolation, normalizeHandle, validateHandle } from "../../../directory-domain";
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const payload = (await request.json()) as { name?: string; slug?: string; color?: unknown; note?: string; status?: "active" | "archived" };
  const db = getDirectoryDb();
  await ensureDirectorySchema(db);
  const current = await db.prepare("SELECT id, name, slug, color, note, status FROM projects WHERE id = ?").bind(id).first();
  ...
  try {
    await db.prepare(`UPDATE projects SET name = ?, slug = ?, color = ?, note = ?, status = ?,
      archived_at = CASE WHEN ? = 'archived' THEN COALESCE(archived_at, CURRENT_TIMESTAMP) ELSE NULL END,
      updated_at = CURRENT_TIMESTAMP WHERE id = ?`).bind(name, slug, color, note, status, status, id).run();
    const project = await db.prepare(`SELECT id, name, slug, color, note, status, created_at AS createdAt,
      updated_at AS updatedAt, archived_at AS archivedAt FROM projects WHERE id = ?`).bind(id).first();

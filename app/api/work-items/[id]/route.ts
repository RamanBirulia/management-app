...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const statements = [db.prepare(`UPDATE work_items SET title = ?, description = ?, parent_id = ?, status = ?, workflow_stage = ?, assignee_id = ?, due_date = ?, design_owner_id = ?, design_draft_url = ?, design_target_date = ?, readiness_note = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
    .bind(payload.title?.trim() ?? current.title, payload.description?.trim() ?? current.description, parentId ?? null, payload.status ?? current.status, payload.workflowStage ?? current.workflowStage, payload.assigneeId === undefined ? current.assigneeId : payload.assigneeId, payload.dueDate === undefined ? current.dueDate : payload.dueDate,
      payload.designOwnerId === undefined ? current.designOwnerId : payload.designOwnerId, payload.designDraftUrl === undefined ? current.designDraftUrl : payload.designDraftUrl?.trim() || null,
      payload.designTargetDate === undefined ? current.designTargetDate : payload.designTargetDate, payload.readinessNote === undefined ? current.readinessNote : payload.readinessNote.trim(), id),

...
export default function AppShell({ active, eyebrow, title, description, children }: { active: "logs" | "directory" | "planning" | "imports" | "context"; eyebrow: string; title: string; description: ReactNode; children: ReactNode }) {
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-note"><span className="pulse" />Release 6 · planning scope</div>
    </aside>
    <section className="content">
      <header className="topbar"><div><p className="eyebrow">{eyebrow}</p><h1>{title}</h1><p className="lead">{description}</p></div><div className="stage-badge"><span className="status-dot" /> Release 6</div></header>
      {children}
    </section>
  </main>;
}

...
export default function ContextPageClient({ kind, id }: { kind: "person" | "project"; id: string }) {
  ...
  return <><section className="context-hero"><div className={`context-avatar ${isPerson ? "person" : "project"}`}>{name.slice(0,2).toUpperCase()}</div><div><p className="eyebrow">{isPerson ? "Person context" : "Project context"}</p><h2>{name}</h2><span className="handle">{handle}</span></div>{record.status === "archived" && <span className="archived-label">архив</span>}{record.note && <div className="context-note"><ReactMarkdown>{record.note}</ReactMarkdown></div>}</section><LogsClient context={{ kind, id }} showComposer={false} /></>;
}

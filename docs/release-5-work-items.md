...
- Справочник и Planning открываются в read-first состоянии без постоянно видимой формы создания.
- Floating-кнопка `+` открывает модальное создание и не перекрывает последнюю карточку благодаря увеличенному нижнему safe area.
- Создание Work Item позволяет выбрать Parent напрямую; быстрый сценарий `Добавить подзадачу` доступен в меню карточки.
- Действия Work Item скрыты в меню `⋯`, чтобы дерево оставалось компактным.
- В интерфейсе явно разделены `Состояние` (`Открыта / Завершена / Отменена`) и `Этап` (`Backlog / Product / Design / PBR / Engineering`).
